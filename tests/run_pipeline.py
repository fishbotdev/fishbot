...
2. To collect oil-capture telemetry, set `TELEMETRY_ON = true` in the *production* copy of
   `FishBot_vX_Y_Z.js` (the copy under test). Without it, steps 1-3 still work but step 4 reports
   nothing.
...
def run_pipeline(
    *,
    ...
    # Step 1: repackage the maps & regenerate the challenge .json files + `base_manifest.json`.
    ...
    run_result_parser.main(commit_sha=commit_sha)

    # Step 4: how well did FishBot capture oil?
    run_telemetry_parser.main(commit_sha=commit_sha)
