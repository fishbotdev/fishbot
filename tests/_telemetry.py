...
The producing side is `multiplay/skirmish/fb_includes_vX_Y_Z/_telemetry.js` - that file is the
...
# The wire format. Must match `TEL_SCHEMA_VERSION` and the `#emit()` template in `_telemetry.js`.
TEL_PREFIX = "TEL|"
