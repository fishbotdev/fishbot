...
def run_autogame(commands: List[str]) -> Tuple[List[str], dict]:
    ...
    #
    # This window has to hold a whole game: FishBot writes one telemetry line per strategy update (6 per
    # minute) on top of its ordinary debug output, and the summary table is printed last. If the window is
    # too small the table scrolls out of reach and the game is silently lost from the batch, so
    # `SCRAPED_CONSOLE_LINES` is sized generously rather than tightly.
    SCRAPED_CONSOLE_LINES = 4000
    console_history = windows_scrape_terminal_history(lines_to_read=SCRAPED_CONSOLE_LINES)
    ...
    # Gets FishBot's own telemetry, which the summary table has no columns for (see `fb_telemetry.py`)
    telemetry = fb_telemetry.parse_console_telemetry(console_history)

    return final_game_summary_table, telemetry
...
def run_tests(test_file_name: str, in_progress_file_path: Path, cycles: int, debug_mode: bool=False) -> float:
    ...
    for i in range(cycles):
        ...
        try:
            final_summary_table, telemetry = run_autogame(AUTOGAME_COMMAND)
        ...
        # FishBot's own telemetry describes the match as a whole, so the same values are attached to every
        # player's row. That keeps one row per player per match, which is what the downstream parser expects.
        for metric, value in fb_telemetry.summarise_telemetry(telemetry).items():
            stats[metric] = value
