...
# Measured on a real 32-minute duel (`temp15.txt`): 385 BRIG + 193 OIL lines. FishBot's own BRIG line is
# the longest at ~148 chars; if the console is narrower than that it wraps onto a second row, which both
# splits the JSON and doubles that line's cost against the budget.
...
def run_autogame(commands: List[str]) -> Tuple[List[str], List[dict]]:
    ...
    # whole console history is scanned rather than just the tail. Empty unless the FishBot copy under
    # test has telemetry enabled (`TELEMETRY_ON`, which follows `DEBUG_MODE_ON`).
...
def run_tests(test_file_name: str, in_progress_file_path: Path, cycles: int, debug_mode: bool=False) -> float:
    ...
    for i in range(cycles):
        ...
        # This is empty (and the file is not created) unless the bot under test has telemetry enabled.
        if telemetry_events:
