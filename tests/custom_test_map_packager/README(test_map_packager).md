...
2. Create a bot-only (bot vs bot) skirmish (challenge) game without a forced human player.
...
1. Set the output directory in `run_test_generator.py`. You can write it directly to Warzone 2100's custom map directory: `%Warzone 2100 Configuration Directory%/maps`.
2. Run `run_test_generator.py`. This will read the maps from `fishbot\tests\custom_test_map_packager\v4.7.0_base_maps` and write the resulting custom-map `.wz` files to the specified output directory from the previous step.  
