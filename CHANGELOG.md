...
- Combat improvements
    - Group cohesion improved:
        - Brigades are more likely to be aggressive when sufficient force is available, more passive otherwise.
        - Group movement now allocates more space for bigger tanks to maneuver, improving chokepoint handling.
        - Group splitting system only creates new brigades when force strength is sufficient.
    - Mortars no longer try to path in front of the main force to attack an invisible target (prefers visible targets).
    - Improved VTOLs being too passive when the match is neck-and-neck (not completely fixed).
- Construction & production fixes
    ...
    - Unit designs & force composition tweaked (bias towards heavy bodies).
    ...
    - Factories no longer overproduce once the unit soft-cap is reached.
- Technical changes
    - Extended E2E test coverage to most base maps shipped with the game as script-generated maps are now testable.
...
- Combat improvements:
    - Complete overhaul of:
        1. Group movement, resulting in smoother group movement with less rubberbanding and better handling of chokepoints.
        2. Group targeting rules, improving focus on objectives and reducing targeting oscillations (further reducing rubberbanding).
- Construction & production fixes:
