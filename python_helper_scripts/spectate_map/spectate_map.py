...
"""
The intent of this file is to create a simple GUI interface to:
    create a new Warzone2100 instance specified by the given `challenge.json` file.
The challenge .json files are generated by `fishbot/tests/run_test_generator.py`.

The GUI interface supersedes the old method of manually modifying .bat files to run these tests.

Please build this file using `build_spectate_map.bat` and run it from the root `fishbot` folder.
This application assumes the local (portable) Warzone 2100 installation is in: `fishbot/Warzone2100/bin/warzone2100.exe`.
"""

# Disclaimer: the rest of this file is AI-generated.
import json
import os
