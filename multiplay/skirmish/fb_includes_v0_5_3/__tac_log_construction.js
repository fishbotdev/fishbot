 ...
 * @returns {{footprint: Coordinate[], clearance: Coordinate[]}} coordinate offsets in the form `[dx, dy]`.
 */
 ...
 * @param {number} x 
 * @param {number} y 
 * @param {(DroidObject | StructureObject)[]} nearbyObjects generated by `enumRange()`.
 * @returns {boolean}
 */
function footprintIsBlocked(structureID, x, y, nearbyObjects) {
	...
	for (let i=0; i<nearbyObjects.length; i++) {
		...
		if (obj.type !== STRUCTURE) 	continue;

		const lookup = STRUCTURES[obj.name];
		const IS_DESIRED_STRUCTURE = obj.player === me && lookup !== undefined && lookup.id === structureID;

		if (IS_DESIRED_STRUCTURE) 	continue;

		const nearbyStructureFootprint = getStructureBounds(lookup.id).footprint;
		for (let j=0; j<nearbyStructureFootprint.length; j++) {
			const ox = obj.x + nearbyStructureFootprint[j][0];
			const oy = obj.y + nearbyStructureFootprint[j][1];

			for (let k=0; k<footprint.length; k++) {
				if (ox === x + footprint[k][0] && oy === y + footprint[k][1]) {
					return true;
				}
			}
		}
...
function buildBaseStructure(taskForceID, structureID, x, y) {
	...
	if (struct.length >= 2) {
		warn(`buildBaseStructure(): failed, somehow there is more than one structure at ${x}, ${y}.`);
		return {status: MISSION_STATUS.FAILED};		
	}

	// Case 1: Nothing exists yet -> build
	if (struct.length === 0) {
		// This function accounts for another structure being built on the intended footprint after mission approval.
		// `structureCanFit()` is not used here as it can return `false` *before* the structure is found by `enumRange()`. 
		// This driver requires these two functions to respond in lockstep (otherwise structures could be constantly be started and abandoned).
		if (footprintIsBlocked(structureID, x, y, nearbyObjects)) {
			deb(`buildBaseStructure(): failed, the footprint of "${structureID}" at ${x}, ${y} is occupied.`);
			return {status: MISSION_STATUS.FAILED};
		}

		// deb(`buildBaseStructure(): Nothing exists at ${x}, ${y} yet; building...`);
	...
	if (struct.length === 1) {
		if (struct[0].status === BUILT) {
			// deb(`buildBaseStructure(): success, ${structureID} is finished`);
			return {status: MISSION_STATUS.SUCCEEDED};
		} else {
			// deb(`buildBaseStructure(): Continuing to build the structure at ${x}, ${y}...`);
