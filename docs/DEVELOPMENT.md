...
* **vs enemy** — FishBot's army strength against its opponents', with the ratio of the two and the distance between the sides' force centres. `1.00` means evenly matched; below it FishBot was fighting outnumbered. Both sides count armed direct-fire units only, so trucks, sensors and repair units do not inflate either number. The ratio is derived from the two figures printed beside it rather than stored, because a division does not survive averaging — averaging per-sample ratios is dominated by the early game, when the opposition holds one or two units and every ratio against it is enormous.
* **indirect** — mortars and artillery on each side, counted apart from strength rather than added to it, because an indirect army fights nothing like a direct-fire one of the same size. Read it with **vs enemy**: an opponent that builds mortars otherwise reads as having almost no army, and the ratio flatters whoever is losing to it.
