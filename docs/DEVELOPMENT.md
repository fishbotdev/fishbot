...
The scraper is Windows-only, but unlike `run_tests.py` no IDE setting is needed. A console attached from a terminal only has a screen buffer as tall as its visible window (~30 rows, a couple of game minutes) because ConPTY — Windows Terminal, VSCode, PyCharm's "Emulate Terminal in Output Console" — keeps its scroll-back in the terminal emulator rather than in the buffer the scraper reads. The script grows that buffer to 9999 rows before starting the game, and runs the game in a console of its own when the terminal will not allow it, in which case the game's output appears in a separate window. A very long FFA can still outrun 9999 rows — the script warns when the start of a game did not survive.
